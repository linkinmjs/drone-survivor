...
| 2026-09-20 | **P2 parte 1 + correcciones commiteado y subido**: `d8b937d` en `origin/master` (139 archivos, +15 293/−2 325), autorizado por el usuario («Hagamos commit y push»). Tanda 3 lanzada: WP-25 (identidad «Última luz») ∥ WP-25b datos (estado `ALERT`, edificio protegido `Building_2_1` «Escuela 12», EN PIE, resultado que lista lo que quedó en pie, ventanas racionadas); después WP-25b pantalla (alerta del taller con mapa del barrio y estática) |
| 2026-09-20 | WP-25 cerrado (identidad «Última luz»): `UIPalette` oscura con 30 pares de contraste verificados, fuentes OFL Chakra Petch / Barlow Semi Condensed / JetBrains Mono con licencia, `ThemeBuilder` de radio 2 px sin sombras con íconos rectangulares y subraya de rotulador, `build_theme` genera también `hud_theme.tres`, carga «del taller», sonidos UI de hojalata, FlightHUD ámbar con indicador de SEÑAL en vez de REC, dron de pieza suelta, `StaticBurst` + estática al morir y al volver, contador de drones del taller, `menu_shots_check` (10 capturas, registrado en los runners). Boot intacto. Checks en verde. Pendientes para el agente de la pantalla de alerta: clave `rec`→`signal` en `GameSettings` (con migración), `GRAPH_ROLL` sin cian, estática entre ALERT e INTRO; para el final: fuentes en `CREDITS.md`/`docs/16`. Notas en `13` §1, `12` §1, `04` §1, `05` §1 |
| 2026-09-20 | WP-25b parte de datos cerrado: estado `ALERT` (6 s + 0,4 s de estática, guarda de entrada 0,35 s, paso máximo 0,25 s), edificio protegido `Building_8_4` «Escuela 12» (bloque medio de 16 m alineado con `EnemySpawn0`, peso ×3 = 8,7 % del barrio), objetivo «PROTEGÉ: ESCUELA 12» que queda fallido si cae, marcador ámbar prioritario, tarjeta que abre por «Qué quedó en pie» y deja puntaje y medalla al final, ventanas racionadas en 5 de 15 manzanas por semilla; `round_check` 16 filas, `city_check` con `ventanas` y `protegido`, `balance_check` en rango. Dos bugs encontrados y corregidos (la alerta se saltaba sola con ventana; el primer cuadro tras instanciar se la comía). Lanzada la parte de pantalla (alerta del taller con mapa del barrio, rama `ALERT` del `CombatHUD`, estática, EN PIE, `rec`→`signal`, `GRAPH_ROLL` sin cian, limpieza de claves) |
| 2026-09-20 | WP-25b parte de pantalla cerrado: `HUDAlertScreen` (monitor del taller: ALERTA, mapa del barrio desde la rejilla, protegido con línea guía, chevron y código del enemigo en cian, «vos», escala, contador), rama `ALERT` del `CombatHUD` con estática de corte a la INTRO, EN PIE, `rec`→`signal` con migración, `GRAPH_ROLL` sin cian, claves huérfanas borradas; `combat_hud_check` 20 filas, `settings_check` con la migración. Suite completa 24/25: `render_check` en rojo por carga externa (otro juego abierto en la máquina: GPU al 86 %); se repite con la máquina libre antes del commit. Movie Maker del arranque real revisado (alerta → estática → INTRO con EN PIE). **Tanda 3 (identidad) completa a falta de esa medición** |
