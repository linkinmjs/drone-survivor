...
> **Nota de P2c, WP-D1 (2026-09-23, ingesta de packs y procedurales, hechos medidos)**: tres packs nuevos entran por el pipeline `objvox` → receta → `compose` → GLB (`tools/voxsplit/models/{foliage,village,city_sample}.json`): `Foliage.rar` (11 piezas de follaje, 2 440 tris, clase **`foliage`** sin cuerpo: raíz `Node3D`, para `MultiMesh`), `VoxelVillagePack.zip` (`lantern`, `fence_picket_3m` de 3,60 m encadenando tres módulos, `barrel_c`, `crate`) y `city-Free Sample.zip` (`bus_stop` 728 tris diezmado ×2, `awning_orange` repintado a (0,95; 0,45; 0,10) con `palette_paint` sobre los índices 62/63/64 del toldo rojo). Novedades del pipeline: `objvox` lee `archivo.rar!miembro` por tubería de `UnRAR p -inul` (ejecutable en `PATH`, rutas de WinRAR o variable `UNRAR`), así que `assets/_raw/` no se extrae dentro del repo; `compose` admite **escala por pieza** (`scale` multiplica el vóxel de esa pieza sin tocar la rejilla: cero triángulos de más), que fue lo que llevó el follaje de la escala del pack (árbol XL de 4,70 m, la altura de una casa) a la del pueblo (×2,5 árboles y tocón: `tree_xl` 11,75 m, `tree_medium` 11,0, `tree_large` 8,5, `tree_small` 3,0; ×2 arbustos y maíz: 1,4 / 2,2 m; ×1,5 pasto y flores); revoxelizar a 5 cm **no sirve** (el paso del OBJ ya es 0,1 m: solo multiplica por ocho las celdas del relleno). Limitación conocida: el VillagePack trae **dos paletas de 256 × 1** (edificios/árboles/rocas y objetos) y el compositor exige una por receta → `chapel` (edificio de 4 × 4 × 6 m, 1 604 tris) queda fuera hasta fundir paletas. Los 19 GLB de Nuke salen byte a byte idénticos; sus sidecars ganan `materials`, `origin`, `scale`, `voxel_size`. **Procedurales** en `tools/build_town_props.gd` (18 piezas, 2 868 tris, `SurfaceTool`, determinista por md5, materiales por familia `assets/city/materials/town_{concrete,sheet,metal,wood,signs}.tres`, atlas `assets/town/signs.png` de 512 × 512 con texto pintado): `gas_station` 17,7 × 25,4 × 8 m (marquesina sobre 4 columnas, 4 surtidores, cartel de precios, tienda con toldo), `water_tower` 16,5 m (622 tris), `silo` 14 m, `field_shed`, `lamp_post`, `bench`, `flag_mast` (bandera a media asta), `monument` (placa «MEMORIA»), `welcome_sign` («BIENVENIDOS» / «ESCUELA 12 · 1,2 KM»), `road_sign_narrow`, `road_sign_speed`, `fence_post`, `fence_wire_6m` (origen `start_x`), `bridge_deck` 20 × 12 m (origen `deck_top_centre`, sin colisión) y cuatro vehículos **placeholder** (`car_a`, `car_b`, `pickup`, `truck`, 168 tris cada uno, `placeholder: true`). **Convención real de las piezas del pueblo: frente en −X** (la puerta de `house_a` se embute en la cara `-x`; `CityGrid` suma `TOWN_YAW_OFFSET = −PI/2` a toda pieza con metadato `town_kind`). Manifiesto `assets/town/pieces_manifest.json` (54 piezas: 14 `building`, 29 `prop`, 11 `foliage`; `{class, scene, footprint, height, origin, front, source, placeholder, tris, shape, surfaces}`) que `TownDesign` y `build_town` fusionan. `town_import_check`: 54 piezas, desvío 0,0 % contra el manifiesto, VRAM 1,34 MB de 2, topes por clase (edificios ≤ 1 500, props ≤ 800 por la parada, follaje ≤ 500), `default_timeout` 180 s.

> **Nota de P2c, tanda 1 (2026-09-22)**: el atlas de `Road_Chunk_5` se midió al construir el viario de cintas: la cara superior es un solo cuadrilátero de 10 × 10 m en `Rect2(0,8563, 0,1508, 0,1353, 0,1353)` con el eje longitudinal sobre la **V**, y el atlas no es repetible (fuera del rectángulo hay caras de otras piezas): las UV de la cinta se parten en celdas enteras de 10 m y nunca se calculan con módulo sobre la posición del mundo (moiré de franjas diagonales). Packs nuevos en `assets/_raw/` (`Foliage.rar`, `VoxelVillagePack.zip`, `city-Free Sample.zip`, `Package.zip`) inventariados en `docs/17` §7; su ingesta por `objvox`/`compose` es WP-D1.

> **Nota del cierre de la revisión de P2b (2026-09-22)**: `objvox`/`compose` endurecidos: errores del OBJ con archivo y línea (`ObjVoxError`, exit 2), índices de vértice validados (el OBJ es 1-based), normal de eje comparada con tolerancia 1e-4 (antes tautológica), pieza vacía tras `crop`/`drop_indices`/`trim` → `ComposeError`, paleta faltante → error, `fill_mismatches` y `colour_conflicts` al `nuke_town_report.txt` y **fallan** en casas (`roof_attic` tiene 1 celda no estanca de 18 759, defecto del OBJ, permitida por `allow_fill_mismatches` en la receta), `KeyError/TypeError/ValueError` del JSON → exit 2, ids duplicados → error, `_require_fits` con `ROOF_EAVES = 6` celdas, diezmado simétrico, `write_emissive_png` validado, cara sin `vt` → error, miembros de ZIP normalizados. Los 19 GLB salen **byte a byte idénticos**. `import_town_piece.gd` ya no muta `texture_filter` de un recurso de disco (lo verifica); `create_shadow_meshes = false` en las 19 piezas (no proyectan sombra y el shadow mesh de vóxel no ahorra nada); `town_import_check` falla con sidecar vacío, exige ≥ 1 índice emisivo y tiene `default_timeout` 120. El shader de daño vive en `city/damage_overlay_body.gdshaderinc` e incluido desde `damage_overlay.gdshader` y `damage_overlay_palette.gdshader` (19 y 34 líneas: solo difiere el `uniform` de la textura).

> **Nota de P2b (2026-09-21, OBJ de MagicaVoxel → rejilla)**: el pack `assets/_raw/nuke Free Sample.zip` (26 OBJ + MTL + PNG de paleta 256×1, sin `.vox`, sin licencia, sin emisivo, sin centrar, caras sin fusionar: `BuildingBlock-0` 48 644 tris) entra por una vía nueva: `tools/voxsplit/objvox.py` voxeliza el OBJ (cada quad alineado a eje marca con su normal la celda interior; **relleno por inundación** porque MagicaVoxel no emite las caras internas: 37 236 celdas de cáscara → 470 428 macizas; 0 conflictos interior/exterior en las 26 piezas) leyendo `zip!miembro`, y `tools/voxsplit/compose.py` + `models/nuke_town.json` componen casas (apilar bloques, techo, puerta/portón, espejo, recorte `crop`), recentran en XZ con `min.y = 0`, escalan ×2,5 (voxel de 5 cm como VoxelCity: puerta 2,20 m, bloque 4,85 × 2,50 m), agrupan la paleta en **familias perceptuales** (el pack está *dithered*: índices 69/70 difieren 4/255 y bloqueaban la fusión: 34 390 → 346 tris) y exportan GLB con `glbwriter` + máscara emisiva `nuke_town_emissive.png` por índices de paleta. Salida: `assets/town/` con 10 casas (`house_a/b/c/d`, `shed` × 2 variantes; 772–2 254 tris, presupuesto 2 400) y 9 props (112–284 tris, ≤ 300 con diezmado), 15 156 tris en total, `nuke_town_palette.png` sin compresión ni mipmaps, materiales `town_houses_opaque/windows.tres` (nearest, MULTIPLY, 0,65), `asset_import/import_town_piece.gd` (material por clase del sidecar `nuke_town.pieces.json`, no por tabla de nombres), escenas heredadas `city/pieces/town/**`, `city/damage_overlay_palette.gdshader` (`filter_nearest`: el shader de daño con mipmaps sangra la paleta) y `BuildingProfile.damage_shader`; `tools/town_import_check` (13 sub-checks + negativa); subcomandos `objinspect` y `town`. Hallazgos: el pack **no tiene ventanas de vidrio**: lo que se enciende son carteles y paneles pegados a la pared (índices 7, 13, 50 → ámbar tibio (198,152,100)), que a la última luz leen como avisos con luz detrás y son lo que raciona `_apply_window_ration()`; `Rooftop-0` no es a dos aguas sino una losa con peto (se usa como ático del bloque angosto); `TrashPile-4/-5` son la misma malla; el importador genera cero niveles de LOD (vértices partidos por cara, como en la ciudad). §1, §2 y §12 (solo `.vox`) quedan ampliados por esta nota; `assets/town/LICENSE-PENDING.md` registra la licencia sin resolver.
