...
> **Nota de P2c, WP-D3/D3b (2026-09-23, arte y legibilidad del pueblo, hechos medidos)**: el primer agente de arte fue detenido a mitad de trabajo y un segundo auditó y cerró; todo se midió con sondas (`tools/legibility_shots.gd`, `town_showcase -- --town` y sondas temporales en `tools/out/wpd3/`). (1) **Follaje**: la paleta del pack venía verde saturado y claro; `palette_paint` en la receta `foliage.json` (índices 9 y 10, dos escalones) deja la copa de `tree_large` a 40 m en **0,80×** la luminancia de una fachada Nuke (objetivo 0,6–0,9; verde oliva con troncos marrón-naranja). Regla operativa nueva: **tocar un PNG o un GLB exige `godot --headless --import` antes de medir u hornear** (fuera del editor Godot no reimporta y usa el `.ctex` viejo: el primer cambio de paleta no se vio). (2) **Cantero y patios**: `plaza_lawn.tres` (rampa ×1,28) deja el cantero en **0,73×** la losa (objetivo ≥ 0,7); patios a 0,62–0,64× la calzada con `CIVIL_DIRT` 0,42 (objetivo ≥ 0,5). (3) **Hormigón procedural**: rampa de `concrete` en `build_town_props.gd` estrechada a 0,68 → desviación estándar de luminancia sobre una placa plana a 30 m **0,030** (≤ 0,04; el silo no tiene cara plana, así que la medida vieja sobre el cilindro no servía: mezclaba moteado, degradado y herrajes y seguirá alta haga lo que haga el material); la chapa ya cumplía (0,031). (4) **Ventanas en picado**: emisión ámbar en `town_houses_windows.tres` y `buildings_00x.tres` → **0,00 %** de píxeles > 0,95 en los cuatro medianos desde 60° a 60 m (objetivo ≤ 2 %), y siguen ámbar a 100 m de frente. (5) **Agua del arroyo**: `CityGrid._build_creek_water` (cinta sobre `creek_points` a terreno del eje + 0,6 m; `city/creek_water.gdshader`: verde-azulado oscuro, fresnel suave, `roughness` 0,15, ondulación por dos senos con `TIME`, `alpha` 0,85, sin reflexiones; los valores viven en los `uniform` del shader, `creek_water.tres` solo dice qué shader); fila de `city_check`: 120 muestras, desvío 0,01 m, 0,24 m bajo el banco más bajo, **2,33 m de aire bajo el tablero**. (6) **Lotes**: `NO_SHADOW_PIECES` en `city_grid.gd` deja solo 6 de 77 mallas proyectando sombra (escuela, hito, `mid_1`, `mid_3`, tanque y silo) → `render_check` HIGH máx **801** (desde 857; B 802, LOW 463; 131,8 fps / p1 86,1). (7) Pad rural espejado corregido en `build_terrain` (`(cos, −sin)`), `field_shed` a −17,18°, pads a ±0,0 mm. (8) Alambrado del disco en el JSON con r 170,6–182,7 y tramos de 20,8–38,3 m: se lee como línea quebrada de postes, no como compás. (9) Farol de la plaza con máscara emisiva ámbar (`village_emissive.png`, `town_village.tres` energía 0,50): 0,00 % quemado, punto ámbar chico. (10) **Niebla de distancia NO aplicada, con números**: desde la ruta a 120 m el pueblo se separa del cielo por 1,035× y del campo por 5,25×: la niebla empuja el pueblo hacia el cielo y sube el campo, comiéndose el contraste que hace el trabajo; la caja del jefe a 200 m desde `DroneSpawn` da 0,379 de media (línea de base si se revisita). `build_environment.gd` y `environment_battle.tres` intactos; SDFGI y tonemapper sin tocar (decisión pendiente del usuario). **Pendiente de decisión de identidad**: `assets/city/materials/props.tres` conserva `emission = (1, 1, 1)` sobre `bake_props_emissive.png` con texels neón (magenta (255, 39, 255) ×894, menta, azul eléctrico, lima, amarillo): el magenta **es el color de los núcleos ventrales del jefe** y compite con el único color que significa «dispará acá» (parche rosa de 0,38 % del cuadro a 100 m, lo más brillante del encuadre); palanca de una línea: teñir la emisión de ámbar como `buildings_001.tres`; se decide en la revisión. Dos bugs cazados por las medidas: `city_check` comparaba el centinela `NAN` de `RoadMesh.surface_y_indexed()` con `is_inf()` (fila en rojo con «quedan nan m» y `expect()` que pasaba sin medir: ahora `is_finite` + `under_deck`), y `CityGrid._within_reach()` recortaba la cinta de agua en el último vértice dentro de los 240 m dejando un tramo seco (vértice de sobra en cada punta). Anotado para la revisión: `town_a.tscn` no es reproducible byte a byte por los 399 `unique_id=` aleatorios que escribe Godot 4.7 (el contenido sí); `plaza_lawn.tres` duplicado con el respaldo de `_ensure_lawn_material()`; las copas leen algo saturadas desde cenital (reciben cielo arriba).

> **Nota de P2b (2026-09-21)**: `ReflectionProbeRig.probe_spots()` trabaja sobre el plano del pueblo (centro, ruta a ±60 m, azotea del mediano más alto, calle lateral a ~100 m, segunda azotea; `ROUTE_REACH`, `SIDE_STREET_REACH`); sin plano degrada a las azoteas más altas y el origen. Las casas del pueblo usan `town_houses_windows.tres` (paleta 256×1, nearest, MULTIPLY 0,65) con carteles retroiluminados en vez de ventanas: en la misma captura son puntos ámbar suaves frente a las ventanas blancas del `block_mid`, por debajo del umbral de deslumbre del checkpoint 4. `tools/legibility_shots`, `overlay_shots`, `environment_shots` y `city_showcase` usan `play_centre()` en vez de `avenue_crossing()`.
