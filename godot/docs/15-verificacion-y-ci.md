...
> **Nota del cierre de la tanda 2 de P2c, segunda vuelta (2026-09-23)**: `balance_check` fila 5 con techo de acierto 0,60 (tres corridas 0,536 / 0,548 / 0,562) y fila 7 con control 340–580 (395,6 y 424,9 s medidos). Las bandas aseveradas se derivan de las constantes (`RANGE_HIT`, `RANGE_CONTROL`); solo `ROW_TITLES` las transcribe. Corrida de confirmación en curso al cerrar esta nota (ver bitácora).

> **Nota del cierre de la tanda 2 de P2c (2026-09-23, suite completa)**: `run_checks.ps1 -Extended` con **32 pasos: 31 en verde** y `balance_check` cortado por el timeout externo de 1 800 s en la semilla 99, porque en paralelo corría un `balance_check` huérfano de un agente y ese agente mató «dos procesos colgados» que incluían el de la suite (`salida −1`). Corrido completo aparte, dos veces, determinista: diez filas en verde y la fila 7 en rojo (control a 395,6 s contra 420–660) → recalibrada a **340–580** (`docs/07` §1) y repetida (ver bitácora). Regla operativa: **un solo Godot pesado a la vez**; antes de lanzar la suite, `Get-Process Godot*` y matar solo lo propio. En `--headless`, `balance_check` imprime `ERROR: Parameter "material" is null` desde el renderizador dummy al retener el haz de asedio: informativo, no cuenta como fallo (lo escribe el motor, no el check).

> **Nota del cierre de la revisión de la tanda 2 de P2c (2026-09-23)**: `city_check` suma «cabos a la distancia declarada» (punta del asfalto y cierre contra nodo + `street_stub_of`, tope 0,30 m), «tablero contra calzada» (tres puntos, ≤ 1 cm), círculo de juego por **huella entera** (`parcel_inside`), «toldo = pila» en 3D, `_rest_y` sin vereda en rurales y `_decor_lots` por nodo entero con `_visual_bounds` reconstruido desde el búfer del `MultiMesh` (**`MultiMesh.get_aabb()`/`VisualInstance3D.get_aabb()` devuelven vacío en `--headless`**: la fila medía todos los grupos con el centro en el origen). `town_plan_check` suma «la firma ve» (cambia un solo valor de giro/escala y la firma cambia) y `POST_Y_MIN/MAX` 2,8–3,2. `terrain_check`: corredor 1× unificado con `build_terrain` y fila de pads atada al conteo de POI con `block: null`. `build_town.gd` deja `town_a.tscn` **byte a byte reproducible** (`_stabilise_ids`). Suite corta en verde tras WP-D4a: `town_import_check`, `terrain_check` (+ negativa 9/9), `city_check` (0 SKIP), `town_plan_check`, `round_check`, `combat_hud_check`, `loading_check`, `hud_projection_check`, `render_check` (HIGH 793), `legibility_shots`, md5 doble del `.tscn`. Pendiente: suite completa `-Extended` con `balance_check` de tres semillas (una semilla sola dio el acierto en el techo de la banda).

> **Nota de P2c, WP-D3b (2026-09-23)**: `city_check` suma la fila **agua del arroyo** (cota del cauce ≤ 0,3 m del terreno del eje + 0,6, bajo el banco exterior, ≥ 1,5 m de aire bajo el tablero con `under_deck` que cuenta las muestras que sí caen bajo la calzada) y corrige el centinela `NAN` (`is_finite`, como los otros 5 sitios del archivo). Regla operativa: **`godot --headless --import` después de tocar PNG o GLB y antes de medir u hornear**; los horneados de `town_a.tscn` difieren solo por los `unique_id=` de Godot 4.7 (normalizándolos son idénticos). Suite corta en verde tras el pase de arte: `town_import_check`, `town_plan_check`, `terrain_check` (+ negativa 9/9), `city_check` (0 SKIP), `round_check`, `combat_hud_check`, `loading_check`, `hud_projection_check`, `render_check` (HIGH 801 / B 802 / LOW 463), `balance_check --only=1`, `legibility_shots` (0,00 % quemado en los ocho encuadres).

> **Nota de P2c, WP-D2 (2026-09-23)**: `town_plan_check` suma filas de plaza (manzana, área, 0 casas, props), arboledas (polígonos, instancias por especie, distancia mínima a la ruta), cercos (tramos, largo, 0 sobre calzada), props, **toldo = pila**, **densidad por anillo** (7 / 16 / 9 / 7 contra bandas 4–7 / 14–22 / 8–14 / 4–8 de `docs/17` §1), determinismo de arboleda (+1 instancia nueva, 0 movidas) y 5 negativas nuevas; la regla de la escuela pasa a «da a una calle, r ≤ 60, ≤ 80 m de la plaza y su normal la mira a ≤ 35°». `city_check` suma `decorado` (grupos de dibujo desde el peor encuadre: 11, tope 12), **hitos visibles** (2 hitos × 5 ojos, el peor subtiende 2,64°: silo desde `EnemySpawn2`, mínimo 0,8°; el ojo mide desde 20 m en los accesos y 40 m en el dron, no desde un peatón: con 1,70 m la primera manzana tapa la mitad de las visuales), **toldo = pila** (4 de 8 puestos bajo toldo, 0 naranjas huérfanos), **ruta dentro del vano** (41 muestras, 0,0 mm de desvío de la recta, 2,52 m de aire bajo el tablero) y densidad por anillo. Deuda de criterio declarada: «≤ 8 lotes por vista» del plan no entra (cinco especies + dos cercos ya son siete): tope 12 con fusión de piezas de < 8 instancias; manda `render_check`. Suite corta en verde: `city_check`, `town_plan_check`, `terrain_check`, `round_check`, `combat_hud_check`, `energy_check`, `loading_check`, `town_import_check`, `render_check` (857), `balance_check --only=1`.
