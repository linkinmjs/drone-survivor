...
> **Nota de P2b (2026-09-21)**: `HUDAlertScreen` dibuja el pueblo desde el plano (`bind_district()`, `_plan_of()` por duck typing): manzanas como polígonos (`draw_colored_polygon` + contorno, relleno 8 %, a oscuras 3,5 %), la ruta como banda (`_draw_route`, **recortada al marco** con Liang–Barsky: entra y sale del pueblo y es más larga que el campo dibujado), el círculo de juego punteado (`_draw_ring`) y el chevrón del enemigo apoyado **sobre el círculo** (`_circle_hit`); `avenue_count()` → `route_segments()`, nuevo `ring_drawn()`; sin plano degrada a marco, norte, escala, protegido, dron y chevrón contra el borde (los carriles ya no existen). §4.2 y la nota de WP-25b quedan corregidas. `combat_hud_check` fila 20 mide el mapa dos veces: contra el barrio de la ronda y contra el doble `tools/fake_town.gd` (12 manzanas, 3 tramos). Captura verificada: doce manzanas irregulares, ruta con dos quiebres, círculo, chevrón sobre el círculo, «Proteger: Escuela 12».

> **Nota de WP-29 (2026-09-20)**: `CombatHUD` cachea la lista de componentes (se armaba por cuadro); `hud_combat` cuesta 0,27 ms por cuadro en HIGH (3,3 %) y **0,24 ms = 11,3 % del cuadro en LOW** (2,12 ms): candidato P5 para las máquinas flojas; `hud_flight` 0,004, `fpv_overlay` 0,009, `camera_rig` 0,002.

> **Nota de WP-28 parte B (2026-09-20)**: el indicador de SEÑAL recibe `FPVOverlay.signal_quality()` desde `DroneRig._feed_hud()` cada frame de física; `lit_bars()` = `ceil(quality · 4)` (4/3/2/1 por 0,75/0,5/0,25) y `CRITICAL_QUALITY` baja de 0,34 a 0,25 para que parpadee solo con una barra. **`"signal"` entra en el preset de HUD `standard`** (WP-25 lo había dejado solo en `full`, como al viejo `rec`): sin eso el jugador no veía la degradación de la señal, que es parte de la identidad elegida. El horizonte del FlightHUD en modo `camera` sigue la sacudida solo (`shake_check` fila 5: < 0,003 px de diferencia en los cuatro modos).

> **Nota de WP-28 parte A (2026-09-20)**: el overlay FPV (`docs/13` §7) vive en la capa −1 bajo el `FlightHUD` y encima del compuesto del ojo de pez (−2); el HUD se lee nítido y con todo su color en todas las capturas (daño 1,0 y EMP 1,0 incluidos). `FPVOverlay.signal_quality()` es la fuente del indicador de SEÑAL: la parte B la publica desde `DroneRig._feed_hud()` con 4/3/2/1 barras por umbrales 0,75/0,5/0,25.

> **Nota del cierre de WP-26 (2026-09-20)**: `Events.hit_confirmed` tiene ahora un cuarto argumento `surface` (`docs/02` §5.1); `CombatHUD` y `combat_hud_check` lo aceptan y el `HitMarker` sigue coloreando por `weak`/`lethal`.

> **Nota de WP-25b parte de pantalla (2026-09-20)**: `HUDAlertScreen` (`hud/combat/alert_screen.gd`, nodo `AlertScreen` entre `IntroBanner` y `GlitchLayer` para que la estática se dibuje encima, hijo `Scanlines` al 50 %) es el **monitor del taller**: «ALERTA» en Chakra Petch 44 parpadeando a 1 Hz (nunca baja de 0,32 de opacidad), mapa de 640×400 con las manzanas de `CityGrid` como rectángulos (relleno 8 %, las manzanas a oscuras 3,5 %), calles como huecos y avenidas más anchas con su cantero, norte arriba (−Z), escala `min(640/extent.x, 400/extent.z)` = 1,47 px/m en el distrito A, barra «100 m», el protegido como caja envolvente girada por su `basis` (lado mínimo 15 px) en ámbar pleno con marco parpadeante y línea guía en codo hasta «Proteger: Escuela 12» (+ «Escuela» en `TEXT_MUTED`), chevron cian proyectado al borde por donde entra el jefe con «ENEMIGO: ARACHNODROID» y trayectoria punteada cian hasta el protegido, rombo «vos» (`ALERT_YOU`) recortado al borde, `ALERT_CONTINUE` y contador en mono. `bind_round()` no lee datos (el distrito no existe hasta `begin()`): `refresh()` los toma al entrar en `ALERT`. `CombatHUD`: `Component.ALERT_SCREEN` al final (18 componentes), `ALERT_COMPONENTS = [ALERT_SCREEN, GLITCH]`, `CINEMATIC_COMPONENTS` suma `GLITCH` (de él cuelga el `StaticBurst` y `advance()` no corre con la capa oculta; no dibuja nada), `_apply_visibility()` elige la lista por `_round_state` y se llama siempre al final de `_on_round_state_changed`; `bind_round()` conecta `RoundManager.alert_finished` → `_glitch.trigger_static(STATIC_SECONDS)` y la ráfaga sobrevive al cambio ALERT→INTRO. `CityBar` rotulada **EN PIE** (`HUD_STANDING`); `HUD_CITY` y `OBJ_DEFEND_CITY_PROGRESS` borradas del CSV. `combat_hud_check` 20 filas (la 20 «alerta del taller» corre primera: 15 manzanas y 2 avenidas, textos traducidos, estática 0,4 s medida a caballo del cambio de estado, vuelta de `CityBar` + `IntroBanner` en INTRO; la 12 compara con `get_title_text()`). §4 (árbol y tabla), §4.2 (listas), §9.2 (tabla del check) y las tablas de claves (`HUD_CITY`, `OBJ_DEFEND_CITY_PROGRESS`, `ALERT_YOU`) quedan corregidas por esta nota. Decisión: el código del enemigo va pegado al chevron (arriba si entra por el norte), no «abajo» como dice la narrativa.

> **Nota de WP-25b parte de datos (2026-09-20)**: `HUDOffscreenMarkers` gana el grupo `protected` con prioridad máxima, rótulo con el nombre propio en **ámbar** (nunca cian) y parpadeo bajo asedio o daño; `HUDObjectiveLine` muestra el objetivo fallido («{0}: CAÍDA» en `DANGER`); la barra de ciudad pasa a rotularse **EN PIE** (`HUD_STANDING`; lo aplica la parte de pantalla). La pantalla de la alerta (`HUDAlertScreen`) y la rama `ALERT` de `CombatHUD` las entrega la parte de pantalla.

> **Nota de WP-25 (2026-09-20)**: el FlightHUD conserva el instrumental pero cambia de estilo: `HUDDraw` con texto ámbar `HUD_TEXT`, contorno de **3 px**, doble trazo +2,0 px, trazo 1,6 px, `font_display()` Chakra Petch / `font_text()` Barlow / `font_mono()` JetBrains Mono, `box()` de radio 2 (`WHITE` queda como alias del ámbar para `objective_hud.gd`); horizonte con peldaños cada 5° numerados cada 10°; cintas laterales con números cada 5 unidades y cursor; compás como tira fina con rumbo en caja; readouts en columna de 200 px; retículo sin anillo; badge de modo rectangular; sticks y RPM con el mismo trazo; **`REC` → `SIGNAL`** (`hud/hud_signal_indicator.gd`: 4 barras + `HUD_SIGNAL`, `FlightHUD.set_signal_quality(0..1)`/`signal_quality()`; hoy fijo en 1,0, WP-28 lo degrada; rótulo `HUD_CFG_SIGNAL`; la clave persistida de `hud_config` sigue siendo `rec` hasta que `GameSettings` la migre). §2.7 (contorno 4 px) y §2.1 (`REC`) quedan corregidos por esta nota. §4.3: `GlitchLayer` gana `trigger_static(seconds)` con `HUDStaticBurst` (`hud/combat/static_burst.gd` + `static.gdshader`: ruido por hash, barras rodantes, caída a negro, reloj por uniform) disparado por `drone_destroyed` (0,8 s) y `drone_respawned` (0,4 s); `RespawnOverlay` muestra «DRON {0} DEL TALLER» (`HUD_TALLER_DRONE`, `RespawnController.get_death_count()`); `combat_hud_check` 19 filas; `set_cinematic(true)` corta la estática (la de ALERT→INTRO la cablea WP-25b).

> **Nota de WP-24e (2026-09-20)**: `RespawnOverlay` muestra el **motivo** de la reconstrucción: con `RespawnController.Reason.ENERGY` agrega la línea `HUD_REBUILDING_NO_BATTERY` («SIN BATERÍA» / «OUT OF BATTERY») en rojo bajo «RECONSTRUYENDO» y baja 34 px el bloque de número y multiplicador; por casco no hay línea. `combat_hud_check` fila 10 cubre los dos casos.

> **Nota de WP-24d parte 2 (2026-09-20)**: `CombatHUD` pasa de 15 a **17 componentes** (`WEAK_HINT` y `COACH` al final del enum; los 15 anteriores conservan su valor): **`HUDWeakPointHint`** bajo `Root` (espacio de pantalla): tras 8 s sin acertar a un punto débil y con alguno expuesto (`WeakPointTracker`, `RefCounted` compartido; el reloj solo corre con algún débil expuesto), corchetes cian de esquina si está en cuadro o flecha hueca al borde si no, con «RODILLA 381 m» encima del marcador (debajo escribe `OffscreenMarkers` su «HOSTIL n m»); se apaga con `hit_confirmed(weak)`. **`HUDCoachTip`** bajo `Frame` a 0,42 del alto: una línea ámbar de 5 s, no bloqueante, una sola vez cada una, disparada por `enemy_attack_telegraphed` (`head_laser` → `HUD_COACH_VISOR`, `emp_pulse` → `HUD_COACH_EMP`, `stomp` → `HUD_COACH_STOMP`) y por 8 s sin acierto (`HUD_COACH_WEAK`); se resetea al entrar en `INTRO`; **regla para componentes de una línea**: el cuerpo de letra se achica (20 → 14) hasta que la traducción entra en su bloque de 600 px porque `HUDDraw.text` recorta en vez de partir, y el check lo vigila en es y en. **`HUDBossBar`** reescrita: rótulos por grupo derivados del `hud_key` del `WeakPointProfile` (`WP_KNEE` → `HUD_WP_GROUP_KNEE`, etc.), `enum State {EXPOSED, COVERED, BROKEN}`, API `state_at/group_count/group_span/group_key/group_label/is_group_exposed/is_group_broken/group_pulse`, destello de 0,55 s cuando un grupo pasa a expuesto (sin destello en la primera evaluación), roto **tachado gris** (`TEXT_DIM`, no rojo), barras 16 px más abajo para el renglón de rótulos. **`HUDIntroBanner`**: cuarta línea en cian `HUD_INTRO_WEAK_HINT` («PUNTOS DÉBILES EN CIAN. TODO LO DEMÁS ES BLINDAJE.»). 16 claves nuevas (`HUD_COACH_*`, `HUD_INTRO_WEAK_HINT`, `HUD_WP_GROUP_*`, `OBJ_BREAK_CORES_*`, `OBJ_COUNT_*`, `OBJ_SELFDESTRUCT_*`; CSV 561 filas). `combat_hud_check`: 17 filas (4 «barra del jefe por grupos», 16 «marcador del punto débil», 17 «consejos contextuales»; captura nueva `coach_hint`). Único solape restante: los corchetes con el retículo cuando el débil cae en el centro exacto (legible, se apaga al acertar).

> **Nota de WP-24c (2026-09-20)**: `hud_projection_check` tiene ahora 15 filas ([0]–[14]) y recorre los **cuatro** modos (OFF/FAST/FAST_WIDE/FULL con 1/1/3/5 `SubViewport`): [11] mide el horizonte en cinco columnas (0,06 / 0,15 / 0,50 / 0,85 / 0,94 del ancho, tolerancia 6 px al centro y 8 a los costados; las columnas en viñeta se saltan según `coverage()`); **[13] cobertura** (headless, rejilla 64×36 + barrido radial: FAST 27,8 % negro y 0 muestras fuera del contrato del frustum, FAST_WIDE y FULL 0 % negro con cobertura mínima 1,000); **[14] churretón y costura** (con ventana: desde `SHOT_POSITION` con guiñada 65° captura FAST, FAST_WIDE y FULL en la misma pose; FULL es la verdad de terreno; la viñeta de FAST no puede tener color donde `coverage()` da 0 (tope 1 %); costura de FAST_WIDE < 0,06 con y sin efectos; y una **prueba negativa** con el `clamp` viejo que inventa imagen en el 79 % de la viñeta). La diferencia media de luminancia contra FULL no detecta el churretón (0,009: la periferia es cielo y suelo lisos); lo que lo caza es el contraste contra `coverage()`. `HUDProjection` no cambió de firma.

> **Nota de WP-22 (2026-09-19)**: `CombatHUD` (`hud/combat/`, 15 componentes sobre `CombatHUDComponent`, paleta provisional `CombatHUDPalette` con los valores de `13` §3 hasta WP-25) vive en la capa 20 de `battle_level.tscn`; `battle_level.gd` lo vincula (`bind_round(manager, sequencer)`, `bind_city`, `bind_drone`) antes de `RoundManager.begin()` y `bind_enemy` tras el spawn. Quien dispara `set_cinematic()` es el propio HUD escuchando `Events.round_state_changed` (si lo llamara `RoundManager` se cerraría un ciclo entre dos `class_name`); `RoundManager.get_combat_hud()` devuelve `CanvasLayer` por el mismo motivo y apaga el HUD al mostrar la tarjeta; la `ObjectiveHUD` provisional ya no se enciende. Ajustes de dibujo respecto de §4.1: el arco de energía va a 130°→230° (a la izquierda; el 220°→320° del texto caía arriba con la convención de `draw_arc`); el aviso de telegrafía baja al 30 % del alto para no pisar la franja de ciudad; la línea de objetivo va al margen izquierdo; el icono de ataque es un triángulo procedural único hasta WP-25. Claves: `HUD_BOSS_PHASE` («FASE {0}») en vez de `BOSS_PHASE_<n>`, `HUD_REBUILDING` + número aparte en vez de `HUD_RESPAWN_IN`, `HUD_RESPAWN_MULTIPLIER` nueva, `HUD_TELEGRAPH_<ATTACK_ID>` con respaldo `HUD_TELEGRAPH_GENERIC`. El orden de las barras del jefe sale del `EnemyProfile` (el de `get_weak_points()` cambia con el reimport). Durante la reconstrucción se ocultan retículo, calor y hitmarker. Cuatro marcadores llevan rótulo, priorizados por importancia (jefe y edificio bajo asedio antes que las pilas). `combat_hud_check` corre sobre el `battle_level` real (con `Global.debug_freeze_ai`), 15 filas con prueba negativa de marcadores NaN/INF/fuera de pantalla, ~35 s. Pendiente para WP-25: dos roces menores de layout (rótulo de marcador con la caja de stick inferior izquierda; bloque `ENERGÍA` con las barras de RPM cuando están encendidas).
