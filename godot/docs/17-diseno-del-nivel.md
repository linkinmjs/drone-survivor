...
|---|---|---|---|---|---|---|---|---|
| `school` | Escuela 12 (protegida, objetivo del jefe) | (−28, −10), manzana 8, fachada al este sobre la calle 4 | 20 × 11 m, 2 plantas | `school` / `tower_b` ×1,15 | bloque bajo y largo con mástil | plaza, ruta a 60 m, calle 4 | ventanas con cartulinas, bicicletas contra la reja, el mástil con bandera a media asta (la puso alguien esta tarde) | pendiente (D2) |
| `plaza` | Plaza central | manzana 9 entera (centroide (25, 6)) | ~45 × 40 m | `plaza` (polígono) | vacío rodeado de árboles | ruta, calles 4 y 5 | bancos, un triciclo volcado, la fuente seca, farolas | pendiente (D2) |
...
| `gas_station` | Estación de servicio «el hola» | manzana 6 (centroide (−129, −42)), sobre la ruta | marquesina 24 × 14 m + tienda 12 × 8 m | `gas_station` (marquesina procedural + `shop_a`) | techo plano flotante sobre 4 columnas, cartel alto | ruta desde el puente (250 m), calle 1, todo el sur | dos autos cargando, uno con la puerta abierta; el cartel de precios apagado | pendiente (D1/D2) |
| `water_tower` | Tanque de agua elevado (hito A) | (135, −20), manzana 5 | 8 × 8 m, 16 m de alto | `water_tower` (procedural) | cilindro sobre 4 patas con escalera; calado | los 4 accesos, ruta, dron | pintado con el nombre del pueblo; goteo | pendiente (D1/D2) |
| `silo` | Silo con galpón (hito B) | (−95, 70), lote rural al sur de la paralela 9 | silo Ø 6 × 14 m; galpón 18 × 10 m | `silo` + `shed` (procedurales) | cilindro macizo con cono | los 4 accesos, ruta, dron | camión de cereal estacionado, tolva a medio cargar | pendiente (D1/D2) |
...
- **Caseríos (anillo 2, este y oeste).** Doce casas crudas (sin `building.gd`) en quintas de 40 × 40 m con cerco de alambre, un árbol grande y un tanque australiano. Están a 160–210 m: se ven, no se rompen, y le dan escala a la salida del pueblo.
...
|---|---|---|---|
| **A** | **vehículos**: 2–3 autos, una camioneta, un camión de cereal, un colectivo | historia ambiental (autos hacia afuera, estación, silo), escala humana | **no hay en ningún pack**; conseguir un pack voxel de vehículos con licencia clara; mientras tanto, cajas procedurales con ruedas (placeholder marcado) |
| **A** | surtidor + marquesina de estación | POI `gas_station` | procedural (`tools/build_town_props.gd`): marquesina, columnas, 2 surtidores, cartel de precios |
| **A** | tanque de agua elevado | hito A | procedural: cilindro sobre 4 patas + escalera |
| **A** | silo con cono | hito B | procedural: cilindro + cono + escalera exterior |
| **A** | alambrado de campo (postes + 3 alambres) y tranquera | límite del área, cierres de cabo | procedural por `MultiMesh` (`RoadMesh.closure`) |
| **A** | cartel de bienvenida y señales de ruta (puente angosto, velocidad, cruce) | legibilidad de la entrada | procedural con un atlas nuevo de 512 × 512 (texto pintado) |
| **B** | galpón de chapa a dos aguas | lote del silo | procedural o recomponer `building3` de `village.vox` |
| **B** | poste de luz con brazo (avenida) | ritmo de la ruta cada 25 m | procedural (el farol del VillagePack es de plaza, no de avenida) |
| **B** | puente vehicular de 12 m de tablero | POI `bridge` | procedural (tablero, estribos, barandas de caño); el puente de madera del VillagePack queda como pasarela peatonal |
| **B** | alcantarilla de hormigón | cierres de cabo `culvert`, cruces de zanja | procedural |
| **B** | bancos de plaza, mástil, monumento | plaza | procedural |
| **B** | capilla | silueta del corazón (opcional, si la plaza queda vacía de un lado) | recomponer desde `village.vox` (edificio de 4 × 4 × 6 m) |
...
| **C** | agua del arroyo (shader) | POI `creek` | shader propio (D3) |
