...
> **Nota del checkpoint 4 (2026-09-21, rebalance)**: `damage` 12 → **16** (blindaje 1,6 con armor 0,90; punto débil **48** ×3,0; una rodilla de 1 600 HP cae con 34 aciertos en vez de 44), `energy_per_shot` 0,45 → **0,30 %**; cadencia, dispersión, calor y bloqueo iguales. Para que el fuego amigo a la ciudad no subiera con el daño, `city_friendly_fire_scale` 0,5 → **0,375** (siguen siendo 6 por impacto; es compensación, no palanca: si `damage` vuelve a moverse, el escalar se mueve en sentido inverso; el respaldo sin perfil de `ProjectilePool` cita 0,375 y 16). §2.9, §2.10 (tabla 16 / 1,6 / 48 / 6), §2.12 (DPS nominal 384, sostenido 208,8, blindaje 7,0; 34 impactos por rodilla de 1 600) y §4/§5 quedan corregidos por esta nota. Medido con el bot: fuego neto 213 → 146 s (= ×36/48), acierto sin cambio (0,51). Fogonazo (`muzzle_flash.tscn`, legibilidad): las **ocho chispas** eran el 100 % del manchón (ablación: luz y quad apagados no cambiaban nada): quad de chispa 0,022 → 0,0121 m, `spread` 26 → 16°, albedo bajo el umbral de glow; luz 4 → 1,5 de energía y 3 → 2 m; llamarada 0,16 → 0,096 m. Resultado: luminancia central con fogonazo +8–10 % → +0,6–0,8 %, píxeles cian del jefe borrados −90 % → −5 % a 60 m, manchón 3 % → 0,14 % del cuadro.
