...
> **Nota del cierre de la tanda 2 de P2c, segunda vuelta (2026-09-23)**: con `RANGE_CONTROL` 340–580 la corrida siguiente dio el control en **424,9 s** (no 395,6: la partida de control tampoco es reproducible al detalle entre procesos, como el resto de la física) y cayó la fila 5 por 0,012: acierto medio **0,562** contra el techo 0,55. Tres corridas completas sobre el mismo pueblo: 0,536 / 0,548 / 0,562 (semillas sueltas 0,53–0,59). La plaza abierta da líneas de tiro más limpias y el bot acierta ~0,05 más: `RANGE_HIT` pasa a **0,33–0,60** como tolerancia, no como balance (mismo criterio que 0,50 → 0,55 en el cierre de la tanda 4). §14: la banda aseverada es 0,33–0,60; el objetivo de diseño 0,35–0,45 queda como estaba, informativo.

> **Nota del cierre de la tanda 2 de P2c (2026-09-23, balance con el pueblo diseñado, hechos medidos)**: `balance_check` completo (tres semillas + control + negativa) corrido dos veces sobre el pueblo final, **determinista** (mismos números en las dos): semillas 1 y 7 victoria en 148 y 127 s con integridad 0,75 / 0,71, 1 muerte cada una, 5 y 3 pilas, acierto sobre puntos débiles 0,587 / 0,584 (promedio de tres semillas **0,536 y 0,548**, dentro de 0,33–0,55: con una sola semilla queda pegado al techo), 2,4 ventanas/min, oro en las dos; **la partida de control** (el coloso arrasa el pueblo sin dron) cae a los **395,6 s** con integridad 0,346 y 25 edificios destruidos (`approach` 189, `siege_beam` 21) contra 568/511 s del pueblo de P2b (`approach` 148, `siege_beam` 29–32) **con el mismo HP** (52 destructibles, 91 800 HP): el coloso camina un 28 % más y tumba el pueblo con un tercio menos de haces porque la plaza abierta le da líneas de tiro y los cinco edificios corridos por la fila del círculo cambian el orden de los blancos. Decisión: se acepta como consecuencia del diseño y `RANGE_CONTROL` pasa de 420–660 a **340–580 s** (mismo ancho; la medida queda a 56 s del piso). El pueblo sin defensa aguanta ~2,7× el tiempo que tarda el bot en ganar; contra los cinco minutos objetivo de §14 y `docs/11` §11 queda un 32 % por encima (antes 70–89 %). §12 y §14 quedan corregidos por esta nota.
