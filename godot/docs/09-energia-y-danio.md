...
> **Nota de P2b (2026-09-22)**: `BatterySpawner` ya no exige puestos propios en `_ready()`: el nivel de batalla arranca sin marcadores y `RoundManager` le adopta los del barrio (`adopt_markers()`) en el mismo cuadro; la comprobación se difiere al primer uso real (`_physics_process`), es `push_warning` una sola vez por ronda (`reset()` la limpia) y `empty_warnings()` la expone al check (`energy_check` 25: 0 avisos al entrar, 1 tras cinco ticks sin adopción; el spawner adoptado queda en 0). §2.5 queda ampliado por esta nota.

> **Nota del checkpoint 4, tercera vuelta (2026-09-21)**: por pedido del usuario **una pila renueva toda la energía**: `battery_amount` 45 → **100** (perfil, default de `EnergyProfile` y de `BatteryPickup`; sin bandera: el recorte de `recharge()` deja 100 % y el número sigue siendo una sola palanca). `energy_check` sub-check 6 espera 100,0 desde 50 % y conserva la sonda de inyección (perfil 12,5). Textos `HELP_ENERGY` y `UI_TIP_BATTERIES` actualizados; el `CombatHUD` no muestra cantidad de pila (nada que cambiar). Bot: pilas/min 1,7 → **1,24** (tope 2,6 sin cambiar), energía mínima 0,23–0,26 (ningún cero: el bot ya no se agota), duración 191 s, muertes 1/2/1 por `stomp`/`pounce`; ninguna banda de `balance_check` cambió. §5 (sub-check 6: 100,0) y §4 Anexo quedan corregidos por esta nota; `BotPilot.energy_hunt_ratio` deja de ser un problema.

> **Nota del checkpoint 4 (2026-09-21, rebalance a favor del jugador)**: el usuario juzgó que la batería duraba poco y que la pila daba poca energía. Valores nuevos (perfil `default_energy.tres` y defaults de `EnergyProfile`): `base_drain` 0,55 → **0,40 %/s**, `throttle_drain` 0,85 → **0,60**, `energy_per_shot` 0,45 → **0,30 %** (en `WeaponProfile`), `battery_amount` +30 → **+45 %**, `respawn_energy_depleted` 20 → **30 %**; `respawn_energy` 60 %, umbral 15 % y EMP −25 % iguales; §2.1, §3.3, §4 y los valores esperados de §5 (96,0 / 90,0 y 93,0 / 44,0 / 95,0 / 30 %) quedan corregidos por esta nota. El riesgo #1 de §6 («autonomía en combate de 35 s») queda **cerrado** con estas tres palancas. `BatterySpawner` inyecta `pickup.amount = profile.battery_amount` (antes eran dos números independientes alineados solo por `energy_check`). Medido con el bot (`balance_check`, tres semillas): pilas recogidas por minuto **3,87 → 1,7–1,8** (se deja de vivir yendo a buscar pilas); la fila 4 de `balance_check` asevera ≤ 2,6 pilas/min. La energía mínima de la partida no sirve como aserción: es el mínimo absoluto y una sola batería agotada la clava en 0 (medias 0,192 / 0,192 / 0,042 con el mismo código); queda impresa como informativa. `BotPilot.energy_hunt_ratio` (0,35) sigue calibrado con la economía vieja: el bot sale tarde a buscar pila y se agota más seguido (muertes 5 → 5 y 8 en dos corridas); no se tocó.

> **Nota del cierre de la revisión (2026-09-20)**: la cabecera de `battery_pickup.gd` describe el prop real y deja anotadas las dos discrepancias abiertas con §3.1: no hay `PickupSound` propio (suena `battery_click` del `AudioPool`) ni `battery_cell.glb` (el prop es procedural).

> **Nota de WP-28 parte B (2026-09-20)**: `Hull.apply_damage()` emite `camera_trauma(0.25, posición del dron)` por impacto recibido y `EnergySystem.apply_emp()` emite `camera_trauma(0.60, posición del dron)`; al morir se suma `destroy_trauma` 1,0 (techo 1,0). La señal de video se degrada con el casco y el EMP (`FPVOverlay.signal_quality()`, `docs/13` §7) y se muestra en el indicador de SEÑAL.

> **Nota de WP-26 (2026-09-20)**: la pila es un prop real procedural (`vfx/battery_cell.tscn`: celda con casquillo en `MultiMesh` de 3 draw calls, emisivo `SUCCESS`, halo de 6 motas en `MultiMesh` sin partículas, luz suave) instanciado por `battery_pickup.tscn` con colisión, capas y API intactas; `assets/drone/battery_cell.glb` (§2.4) no existe ni hace falta. `battery_collected` → `pickup_flash` (1 emisor); el `PickupSound` de §3.1 lo cubre el `AudioPool` con `battery_click` (WP-27). `drone_damaged` → `drone_sparks` anclado al dron; `drone_destroyed` → `drone_burst` de 0,6 s antes de la estática.

> **Nota de WP-24e (2026-09-20)**: la **recarga en reposo se apaga** (`idle_recharge 0`, `idle_recharge_cap 0`; campos conservados): el usuario juzgó que esperar 1 %/s hasta 10 % no era una buena experiencia. **Batería a 0 % = reconstrucción** como una muerte: `RespawnController` escucha `EnergySystem.depleted`, `enum Reason {HULL, ENERGY}`, `get_reason()`, `force_respawn(reason)`; en el caso ENERGY emite `Events.drone_destroyed(position)` una sola vez (cuenta como reconstrucción: −300 y ×0,6) y llama al nuevo `Hull.deactivate()` para que un barrido del jefe no publique un segundo `drone_destroyed` durante los 12 s; `_finish()` restaura casco y energía como siempre. Energía al volver: `EnergyProfile.respawn_energy_depleted = 20 %` por batería, `respawn_energy = 60 %` por casco. Un EMP que deja la batería en 0 % dispara la reconstrucción. `energy_check` sub-checks 5, 9, 10 y 16 reescritos (`DOC_RESPAWN_ENERGY_DEPLETED`); `balance_check` en rango sin tocar el bot (3/3/1 muertes, energía mínima 4 %). §2.1, §2.2, §2.8, §2.9, §3.3, §3.5 y §5 quedan corregidos por esta nota.
