...
| Foliage, VoxelVillagePack y city-Free Sample (packs de MagicaVoxel, autores no identificados) | `assets/_raw/Foliage.rar`, `assets/_raw/VoxelVillagePack.zip`, `assets/_raw/city-Free Sample.zip` (**sin readme ni licencia**) | follaje (11 piezas), farol, cerco, barril, cajón, parada de colectivo y toldo | **Pendiente (P2c, 2026-09-23)**: identificar autor y términos; en el repo solo quedan los GLB derivados en `assets/town/` (recetas `tools/voxsplit/models/{foliage,village,city_sample}.json`), los archivos crudos siguen fuera de git; ver `assets/town/LICENSE-PENDING.md`. `Package.zip` (barrancas) no se usa. Las 18 piezas procedurales de `tools/build_town_props.gd` y el atlas `signs.png` son propios |
